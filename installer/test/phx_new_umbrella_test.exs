...
defmodule Mix.Tasks.Phx.New.UmbrellaTest do
  ...
  test "new with umbrella and defaults" do
    in_tmp("new with umbrella and defaults", fn ->
      ...
      assert_file(web_path(@app, "assets/js/app.js"), ~s[import socket from "./socket"])
      assert_file(web_path(@app, "assets/js/socket.js"), ~s[import {Socket} from "phoenix"])
  ...
  test "new with mysql adapter" do
    in_tmp("new with mysql adapter", fn ->
      ...
      assert_file(
        "custom_path_umbrella/apps/custom_path/test/support/data_case.ex",
        "Ecto.Adapters.SQL.Sandbox.mode"
      )

      assert_file(
        "custom_path_umbrella/apps/custom_path_web/test/support/channel_case.ex",
        "Ecto.Adapters.SQL.Sandbox.mode"
      )

      assert_file(
        "custom_path_umbrella/apps/custom_path_web/test/support/conn_case.ex",
        "Ecto.Adapters.SQL.Sandbox.mode"
      )
    end)
  end
  ...
  describe "web task" do
    ...
    test "generates web-only files", %{tmp_dir: tmp_dir} do
      in_tmp(tmp_dir, fn ->
        ...
        assert_file("another/lib/another/views/page_view.ex", ~r/defmodule Another.PageView/)
        ...
        assert_file("another/assets/js/app.js", ~s[import socket from "./socket"])
        assert_file("another/assets/js/socket.js", ~s[import {Socket} from "phoenix"])
