defmodule Phoenix.Router.Helpers do
    ...
    code =
      quote do
        ...
        defp segments(segments, query, reserved) do
          dict = for {k, v} <- query, (k = to_string(k)) not in reserved, do: {k, v}
  ...
  defp expand_segments(segments) when is_list(segments), do: expand_segments(segments, "")
  ...
  defp expand_segments([{:|, _, [h, t]}], acc),
    do: quote(
      do: unquote(expand_segments([h], acc)) <>
  ...
  defp expand_segments([h | t], acc),
    do: expand_segments(
      t,
      quote(do: unquote(acc) <> "/" <> URI.encode(to_param(unquote(h)), &URI.char_unreserved?/1))
    )

  defp expand_segments([], acc), do: acc
end
