...
defmodule Phoenix.Integration.LongPollTest do
  ...
  defp encode("2." <> _ = _vsn, map) do
    Poison.encode!([map["join_ref"], map["ref"], map["topic"], map["event"], map["payload"]])
  end
  ...
  def join(path, topic, vsn, :pubsub, payload) do
    ...
    %{
      "token" => Phoenix.Token.sign(Endpoint, Atom.to_string(__MODULE__), {
        :v1,
        "unknown",
        pid,
        topic
  ...
  for {serializer, vsn} <- [{LongPollSerializer, "1.0.0"}, {V2.LongPollSerializer, "2.0.0"}] do
    @vsn vsn
    @join_ref "123"
    ...
    describe "with #{vsn} serializer #{inspect(serializer)}" do
      ...
      test "filter params on join" do
        log =
          capture_log(fn ->
            join("/ws/logging", "room:lobby", @vsn, :local, %{
              "foo" => "bar",
