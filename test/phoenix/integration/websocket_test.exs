...
defmodule Phoenix.Integration.WebSocketTest do
  ...
  for {serializer, vsn, join_ref} <- [
        {WebSocketSerializer, "1.0.0", nil},
        {V2.WebSocketSerializer, "2.0.0", "1"}
      ] do
    ...
    describe "with #{vsn} serializer #{inspect(serializer)}" do
      ...
      test "join, leave, and event messages" do
        ...
        assert_receive %Message{
          event: "new_msg",
          payload: %{"transport" => "Phoenix.Transports.WebSocket", "body" => "hi!"}
        }
      ...
      test "logs and filter params on join and handle_in" do
        ...
        {:ok, sock} =
          WebsocketClient.start_link(
            self(),
            "ws://127.0.0.1:#{@port}/ws/logging/websocket?vsn=#{@vsn}",
            @serializer
          )
      ...
      test "refuses connects that error with 403 response" do
        assert WebsocketClient.start_link(
                 self(),
                 "ws://127.0.0.1:#{@port}/ws/websocket?vsn=#{@vsn}&reject=true",
                 @serializer
               ) == {:error, {403, "Forbidden"}}
      end
      ...
      test "shuts down when receiving disconnect broadcasts on socket's id" do
        {:ok, sock} =
          WebsocketClient.start_link(
            self(),
            "ws://127.0.0.1:#{@port}/ws/websocket?vsn=#{@vsn}&user_id=1001",
            @serializer
          )
      ...
      test "returns 403 when versions to not match" do
        ...
        assert log =~
                 "The client's requested channel transport version \"123.1.1\" does not match server's version"
      end
